...
class LearningCurve(object):
    ...
    def _run_a_single_trial(self, X_pool, y_pool, X_test, y_test, al_strategy, classifier_name, bootstrap_size,  step_size, budget, t, folderpath,lengthoftxt=None):
        ...
        file_name_parameter = "./unc_MultinomialNB/Index_2Biggest_Claasifier_Trial" + str(t + 1) + ".txt"
        ...
        while len(trainIndices) < budget and len(pool) >= step_size:
            ...
            model = get_classifier(classifier_name,parameterslist[int(parameters[ite,0])])
            ...
            if tempaccuracy < lastaccuray:
                model = get_classifier(classifier_name,parameterslist[int(parameters[ite,1])])
                model = eval(model)
...
if __name__ == '__main__':
    ...
    folderpath = './' +args.strategies + '_' + args.classifier + '_Best2Classifier/'
